...
function buildAbilityGraph(merged) {
  ...
  for (const f of fs.readdirSync(dir).sort()) {
    ...
    // Extract owner safely across schemas
    let owner = '';
    if (Array.isArray(p.users)) owner = p.users.join(', ');
    else if (p.owner) owner = p.owner;
    else if (Array.isArray(snap.users)) owner = snap.users.join(', ');
    else if (snap.owner) owner = snap.owner;
    abilities.push({
      id,
