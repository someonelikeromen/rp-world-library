...
2. **角色卡**：优先读取 `card/` 中已有 JSON；没有则提示用户放入。
3. **文风**：检查 `style/`；无指定则默认"对话驱动、细腻独白、场景推进明确"。
4. **世界书**：确认使用的世界观，按 `rp-world-search` skill 定位世界书。
5. **记忆**：续档读 `memory/`；新档初始化。
...
2. 1-3 段概括中间变化。
3. 更新 `memory/project.md` 和相关人物状态。
4. 在新时间点给出可互动场景。
