# Locked Plan: FGO Lostbelt 5.1-5.2 P1 Prep Gate

Date: 2026-07-13
Step id: `prep-lb5`
Pilot root: `campaigns/world-library/manual-curation/type-moon-p1-hybrid/fgo-lb5-pilot/`
Required model for later execution agents: `lt-yuyu/gpt-5.5`

## Objective

Prepare only the Lostbelt 5.1-5.2 prep gate scaffold, source inventory, source classification report, source-unit manifest, execution log, status, and two chapter-extraction work packets for entries 97-98 of `[沙盒]FGO 0.8.worldbook.json`.
...
|---:|---|---|---|---:|---:|---|
| 97 | `fgo-lb5-1-atlantis` | `lb5-1-atlantis` | `FGO_LB5_1_亚特兰蒂斯` | 31 | 37 | `[沙盒]FGO 0.8.worldbook.json:12566-12567` |
| 98 | `fgo-lb5-2-olympos` | `lb5-2-olympos` | `FGO_LB5_2_奥林波斯` | 29 | 41 | `[沙盒]FGO 0.8.worldbook.json:12689-12690` |
...
1. `world_state`
2. `stage_characters`
3. all raw `-事件X` timeline events, preserving irregular codes such as `事件A1`, `事件B1`, and `事件D3.5`
4. raw event code, title, raw time, trigger, completion, orderIndex, summaryRaw, and sourceRefs
...
This prep stage is complete only after the seven named scaffold artifacts and exactly two `work-packets/lostbelt/*.json` files for LB5.1 and LB5.2 are written under the pilot root. No extraction, normalization, merge, source text, graph, timeline, source import change, or old curated change is authorized.
