# FGO Lostbelt 6.1-6.2 Source Classification Report
...
The Lostbelt 6.1-6.2 prep gate uses entries 100 and 117 of `[沙盒]FGO 0.8.worldbook.json` as script-primary source units.
...
|---:|---|---|---|---|---|
| 100 | `FGO_LB6_1_阿瓦隆前篇` | `fgo-lb6-1-avalon-pre` | script-like | script-primary | `[沙盒]FGO 0.8.worldbook.json:12931-12936` |
| 117 | `FGO_LB6_2_圆桌后篇` | `fgo-lb6-2-round-table-post` | script-like | script-primary | `[沙盒]FGO 0.8.worldbook.json:15082-15087` |
...
The same entries also appear in raw mirror portions of the worldbook at `[沙盒]FGO 0.8.worldbook.json:12975-12976` and `[沙盒]FGO 0.8.worldbook.json:15126-15127`; the selected primary references are the main entry comment/content lines above.

## Prep Observations

| entryIndex | event code range | observed event markers | observed dialogue quotes | note |
|---:|---|---:|---:|---|
| 100 | duplicated `事件A0`, `事件A1-事件A6`, `事件A-事件Y` | 33 | 27 | duplicated raw `事件A0` must be preserved; source mixes 妖精历, 女王历, and 2021 actual time |
| 117 | `事件A-事件L` | 12 | 19 | late-collapse chapter must remain a distinct packet; do not fold into LB6.1 |
...
No missing, duplicated-entry, split-entry, or ambiguous-entry blocker was found for the requested target entries. There is an internal duplicated raw event code (`事件A0`) inside LB6.1; it is not a prep blocker, but it is a downstream extraction/audit requirement.
