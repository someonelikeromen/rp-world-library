# Locked Plan: FGO Lostbelt 6.1-6.2 P1 Prep Gate

Date: 2026-07-13
Step id: `prep-lb6`
Pilot root: `campaigns/world-library/manual-curation/type-moon-p1-hybrid/fgo-lb6-pilot/`
Required model for later execution agents: `lt-yuyu/gpt-5.5`

## Objective

Prepare only the Lostbelt 6.1-6.2 prep gate scaffold, source inventory, source classification report, source-unit manifest, execution log, status, and two chapter-extraction work packets for entries 100 and 117 of `[沙盒]FGO 0.8.worldbook.json`.
...
|---:|---|---|---|---:|---:|---|
| 100 | `fgo-lb6-1-avalon-pre` | `lb6-1-avalon-pre` | `FGO_LB6_1_阿瓦隆前篇` | 33 | 27 | `[沙盒]FGO 0.8.worldbook.json:12931-12936` |
| 117 | `fgo-lb6-2-round-table-post` | `lb6-2-round-table-post` | `FGO_LB6_2_圆桌后篇` | 12 | 19 | `[沙盒]FGO 0.8.worldbook.json:15082-15087` |
...
- Curated story mirror: comparison/mirror only; do not overwrite or use as a formal event source.
...
1. `world_state`
2. `stage_characters`
3. all raw `事件X` timeline events, preserving irregular / duplicated raw codes such as the repeated `事件A0` in LB6.1
4. raw event code, title, raw time, triggerRaw, completionRaw, orderIndex, summaryRaw, and sourceRefs
...
12. `sourceRefs`, `sourceType`, `credibility`, and `canonStatus` on formal records
...
- duplicate raw event codes must be preserved while downstream file IDs remain unique and auditable;
...
- valid JSON outputs;
- no unsupported formal labels; source-first terminology must be preserved;
- no formal aggregate `characters.json`, `events.json`, or `relationships.json` as the fact store;
- later merge must preserve `triggerRaw`, `completionRaw`, and `summaryRaw` for every event;
- later relationship edges must preserve `timelineId` and `timeRange`;
...
- Merge gate: closed until normalized fragments pass audit and event raw fields are retained.
- Timeline / graph gate: closed until canonical source layer exists, passes audit, and relationship edges carry `timelineId` / `timeRange`.
...
This prep stage is complete only after the seven named scaffold artifacts and exactly two `work-packets/lostbelt/*.json` files for LB6.1 and LB6.2 are written under the pilot root. No extraction, normalization, merge, source text, graph, timeline, source import change, or old curated change is authorized.
