...
核心原则：**当前用户已授权 Two-Pass Runtime 轨道可持续分批自动推进直到完成；自动执行不能跳过 review，不能越过高风险确认点，不能污染 RP 剧情状态。**
...
这个循环允许 Pi 在 Two-Pass Runtime 轨道内长期推进；每批之间不需要重复人工确认，但高风险确认点必须暂停请求用户确认。
...
当前用户已授权本轨道自动 `orch_start` 后续批次；但高风险确认点、发布版修改、剧情状态写入和 release 同步仍不得自动执行。
...
以下动作即使在长期自动迭代授权下，也必须单独向用户确认，不得作为普通任务顺带执行：
