...
兑换系统是主角专属的可选外挂。它使用多世界战斗框架为完整兑换项定价：先按 `02-rating/02-evaluation-method.md` 的评价方式完成扒皮估价，再按 `02-rating/03-rating-system.md` 的 N0–N24 评级体系确定层级，最后用多级“奖励点”支付。
...
6. 读取 `02-rating/02-evaluation-method.md`，按评价方式拆证据、区分常态/峰值/条件、排除称号水分，形成评价记录。
7. 读取 `02-rating/03-rating-system.md`，将评价结果映射为兑换项自身 N 层级。
8. 按价格表计算价格。
9. 展示面板，等待用户确认。
10. 扣除奖励点并写入交易。
11. 按类型落盘到角色卡模块。
12. 更新 `memory/world-history.md`。
...
- `evaluate`：检查兑换项是否包含多世界评价框架估价记录，尤其是 `evaluationMethod: "multi-world-evaluation-method-v1"` 或等价记录。
- `price`：已知 N 层级时查表；只查价格，不负责评价。
- `quote`：校验 + 估价记录检查 + 报价；不得跳过评价方式直接报价。
